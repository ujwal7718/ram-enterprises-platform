      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {capabilities.map((cap, index) => (
            <motion.div 
              key={index}
              className={`bg-[#1A365D]/30 backdrop-blur-sm border border-[#1A365D] p-8 rounded-xl hover:bg-[#1A365D]/60 hover:border-[#00B4D8]/50 transition-all duration-300 flex flex-col group ${index === 3 ? 'lg:col-start-1 lg:ml-auto w-full lg:max-w-md' : ''} ${index === 4 ? 'lg:col-start-2 lg:mr-auto w-full lg:max-w-md' : ''}`}
