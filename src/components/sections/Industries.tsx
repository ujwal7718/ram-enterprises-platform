      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {sectors.map((sector, index) => (
            <div 
              key={index} 
              className="bg-white p-5 rounded-lg border border-gray-100 hover:border-[#00B4D8] hover:shadow-md transition-all duration-300 group flex items-center justify-between cursor-default"
            >
              <span className="font-medium text-[#1A365D] group-hover:text-[#00B4D8] transition-colors">
                {sector}
              </span>
              <ChevronRight size={18} className="text-gray-300 group-hover:text-[#00B4D8] transition-colors shrink-0" />
            </div>
          ))}
        </div>
